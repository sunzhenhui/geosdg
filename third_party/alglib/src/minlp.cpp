...
*************************************************************************/
void minlpsolversetbbsyncsubsolveripm(minlpsolverstate &state, const ae_int_t memlen, const xparams _xparams)
{
    jmp_buf _break_jump;
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    ...
    if( _xparams.flags!=(alglib_impl::ae_uint64_t)0x0 )
        ae_state_set_flags(&_alglib_env_state, _xparams.flags);
    alglib_impl::minlpsolversetbbsyncsubsolveripm(state.c_ptr(), memlen, &_alglib_env_state);
    alglib_impl::ae_state_clear(&_alglib_env_state);
    return;
}
...
INPUT PARAMETERS:
  ...
  -- ALGLIB --
     Copyright 01.02.2026 by Bochkanov Sergey
*************************************************************************/
void minlpsolversetbbsyncsubsolversqp(minlpsolverstate &state, const xparams _xparams)
{
    jmp_buf _break_jump;
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    ...
    if( _xparams.flags!=(alglib_impl::ae_uint64_t)0x0 )
        ae_state_set_flags(&_alglib_env_state, _xparams.flags);
    alglib_impl::minlpsolversetbbsyncsubsolversqp(state.c_ptr(), &_alglib_env_state);
    alglib_impl::ae_state_clear(&_alglib_env_state);
    return;
}
...
*************************************************************************/
void minlpsolvermarkaslinearvar(minlpsolverstate &state, const ae_int_t k, const xparams _xparams)
{
    jmp_buf _break_jump;
    alglib_impl::ae_state _alglib_env_state;
    alglib_impl::ae_state_init(&_alglib_env_state);
    ...
    if( _xparams.flags!=(alglib_impl::ae_uint64_t)0x0 )
        ae_state_set_flags(&_alglib_env_state, _xparams.flags);
    alglib_impl::minlpsolvermarkaslinearvar(state.c_ptr(), k, &_alglib_env_state);
    alglib_impl::ae_state_clear(&_alglib_env_state);
    return;
}
    ...
    const xparams _xparams)
{
    ...
    alglib_impl::rcommv2_request request(&optimizer.rcommv2, ptr, "minlpsolver");
...
callbacks.fvec = fvec;
    ...
    while( alglib_impl::minlpsolveriteration(state.c_ptr(), &_alglib_env_state) )
    {
        ...
        alglib_impl::ae_assert(ae_false, "ALGLIB: critical error in 'minlpsolveroptimize' (RCommV2 request escaped handler)", &_alglib_env_state);
...
static void bbgd_subproblemcopyasunsolved(const bbgdsubproblem* src,
     ...
     bbgdsubproblem* dst,
     ae_state *_state);
static void bbgd_subproblemmergeinsolution(bbgdsubproblem* subproblem,
     /* Real    */ const ae_vector* x,
...
ae_bool bbgditeration(bbgdstate* state, ae_state *_state)
{
    ...
    bcopyallocv(n, &state->islinear, &state->rootsubproblem.parentlinearity, _state);
...
lbl_0:
    goto lbl_3;
lbl_4:
    ae_assert((state->front.frontstatus==bbgd_stsolved||state->front.frontstatus==bbgd_sttimeout)||state->front.frontstatus==bbgd_stunbounded, "BBGD: integrity check 184017 failed", _state);
    ...
    if( state->dolaconictrace )
    {
        ...
        state->lastlaconicreportepoch = ae_ifloor(stimergetmsrunning(&state->timerglobal, _state)/(double)bbgd_laconicreportperiod, _state);
    }
...
lbl_1:
    goto lbl_7;
lbl_8:
    ae_assert((state->front.frontstatus==bbgd_stsolved||state->front.frontstatus==bbgd_sttimeout)||state->front.frontstatus==bbgd_stunbounded, "BBGD: integrity check 826253 failed", _state);
    ...
    if( state->front.frontstatus==bbgd_stunbounded )
    {
        if( state->doanytrace )
        {
            ae_trace("> unboundedness was signaled, the problem is likely to be unbounded (suspiciously large |x| or -f)\n");
        }
...
lbl_5:
    ...
    if( state->front.frontstatus==bbgd_stunbounded )
    {
        if( state->doanytrace )
        {
            ae_trace("> unboundedness was signaled during solution of the root subproblem\n");
        }
        state->unbounded = ae_true;
    }
lbl_6:
...
lbl_12:
    ...
    if( state->doanytrace )
    {
        ae_trace("> a primal solution is found: f=%0.9e, relative duality gap is %0.3e; %0.3fs passed\n",
            (double)(state->repf),
...
lbl_9:
     ...
     */
    ae_assert((state->front.frontstatus==bbgd_stsolved||state->front.frontstatus==bbgd_sttimeout)||state->front.frontstatus==bbgd_stunbounded, "BBGD: integrity check 280023 failed", _state);
    state->repterminationtype = -3;
    if( state->timedout )
    {
    ...
    if( state->doanytrace )
    {
        ...
        {
            ae_trace("> the problem is infeasible (or feasible point is too difficult to find); %0.3fs passed\n",
                (double)(0.001*stimergetmsrunning(&state->timerglobal, _state)));
        }
    }
    if( state->doanytrace )
    {
        if( state->repterminationtype==-4 )
        {
            ae_trace("> the problem is unbounded; %0.3fs passed\n",
                (double)(0.001*stimergetmsrunning(&state->timerglobal, _state)));
        }
    }
lbl_10:
     ...
     ae_state *_state)
{
    ...
    state->userterminationneeded = ae_false;
    state->maxsubsolvers = icase2(ae_is_trace_enabled("DBG.BBSYNC.ONESUBSOLVER"), 1, 4*maxconcurrency(_state), _state);
     ...
     ae_state *_state)
{
    ...
    if( *requesttype==0 )
    {
        ...
        *querysize = 0;
        ae_assert(subsolver->rcommv2.queryfuncs>=1, "BBGD: integrity check 946245 failed", _state);
        ae_assert(subsolver->rcommv2.queryvars>=1, "BBGD: integrity check 947246 failed", _state);
        ae_assert(subsolver->rcommv2.querydim==0, "BBGD: integrity check 947247 failed", _state);
    ...
    ae_assert(*queryfuncs==subsolver->rcommv2.queryfuncs, "BBGD: subsolvers send requests that have incompatible sizes and can not be aggregated", _state);
    ae_assert(*queryvars==subsolver->rcommv2.queryvars, "BBGD: subsolvers send requests that have incompatible sizes and can not be aggregated", _state);
    ae_assert(subsolver->rcommv2.querydim==0, "BBGD: subsolver send request with QueryDim<>0, unexpected", _state);
    ae_assert((localrequesttype!=3&&localrequesttype!=5)||*queryformulasize==subsolver->rcommv2.queryformulasize, "BBGD: subsolvers send requests that are incompatible due to different query formula sizes", _state);
    n = *queryvars;
    ...
    if( localrequesttype==1 )
    {
         ...
         */
        rgrowv(*querysize*(*queryvars)+subsolver->rcommv2.querysize*(*queryvars), querydata, _state);
        rcopyvx(subsolver->rcommv2.querysize*n, &subsolver->rcommv2.querydata, 0, querydata, *querysize*(*queryvars), _state);
     ...
     ae_state *_state)
{
    ...
    ae_assert(subsolver->rcommv2.queryfuncs==queryfuncs, "BBGD: integrity check 041003 failed", _state);
    ae_assert(subsolver->rcommv2.queryvars==queryvars, "BBGD: integrity check 042003 failed", _state);
    ae_assert(querydim==0, "BBGD: integrity check 043003 failed", _state);
    ae_assert(*requestidx+subsolver->rcommv2.querysize<=querysize, "BBGD: integrity check 044003 failed", _state);
    n = queryvars;
    ...
    if( state->lastrequesttype==1 )
    {
        ...
        rcopyvx(subsolver->rcommv2.querysize*queryfuncs, replyfi, fisrcoffs, &subsolver->rcommv2.replyfi, fidstoffs, _state);
        sparsecreatecrsfromcrsrangebuf(replysj, jacsrcoffs, jacsrcoffs+subsolver->rcommv2.querysize*queryfuncs, &subsolver->rcommv2.replysj, _state);
    ...
    if( state->lastrequesttype==2 )
    {
        ...
        rcopyvx(subsolver->rcommv2.querysize*queryfuncs, replyfi, fisrcoffs, &subsolver->rcommv2.replyfi, fidstoffs, _state);
        rcopyvx(n*subsolver->rcommv2.querysize*queryfuncs, replydj, jacsrcoffs, &subsolver->rcommv2.replydj, jacdstoffs, _state);
        *requestidx = *requestidx+subsolver->rcommv2.querysize;
        return;
    }
     ...
     ae_state *_state)
{
    ...
    bcopyallocv(src->n, &src->subproblemlinearity, &dst->subproblemlinearity, _state);
}
     ...
     ae_state *_state)
{
    ...
    ae_assert(!(isintfeas&&ae_fp_greater(earlyerror,(double)(0))), "BBGD: 491152 failed", _state);
    if( !subproblem->hasdualsolution||ae_fp_less(f-earlyerror,subproblem->bestfdual-subproblem->bestfdualearlyerror) )
    {
    ...
    {
        ae_assert(ae_fp_eq(earlyerror,(double)(0)), "BBGD: 511154 failed", _state);
        subproblem->hasprimalsolution = ae_true;
        rcopyallocv(subproblem->n, x, &subproblem->xprim, _state);
        subproblem->fprim = f;
        subproblem->hprim = sclfeaserr;
    }
    bestworstspread = ae_fabs(subproblem->bestfdual-subproblem->worstfdual, _state);
    subproblem->dualbound = subproblem->bestfdual-subproblem->bestfdualearlyerror-bbgd_safetyfactor*bestworstspread;
    for(i=0; i<=subproblem->n-1; i++)
    {
        subproblem->subproblemlinearity.ptr.p_bool[i] = subproblem->subproblemlinearity.ptr.p_bool[i]&&ae_fp_less_eq(nlrep->ptr.p_double[i],bbgd_nonlinearitythreshold);
    }
}
     ...
     ae_state *_state)
{
    ...
    if( state->dotrace )
    {
        ...
        ae_trace(", fprim=%0.12e)",
            (double)(subproblem->fprim));
    }
    if( (state->hasprimalsolution&&!subproblem->donotfathom)&&ae_fp_greater_eq(subproblem->dualbound,state->fprim-state->pdgap*rmaxabs2(state->fprim, (double)(1), _state)) )
    {
        if( state->dotrace )
        {
            ae_trace(", fathomed\n");
     ...
     ae_state *_state)
{
    ...
    {
        state->ffdual = ae_minreal(state->ffdual, state->fprim-(double)10*state->pdgap*rmaxabs2(state->fprim, (double)(1), _state), _state);
    }
    ae_frame_leave(_state);
}
...
lbl_1:
    ...
    if( front->rstate.rcomm2_handler!=NULL && front->rstate.requesttype!=0 && front->rstate.requesttype<=_ALGLIB_MAX_RCOMMV2_REQUEST )
        front->rstate.rcomm2_handler(&front->rstate, front->rstate.handler_p0, front->rstate.handler_p1, front->rstate.handler_p2, front->rstate.handler_p3, _state);
    else
        goto lbl_rcomm;
lbl_0:
    goto lbl_1;
     ...
     ae_state *_state)
{
    ...
    if( front->frontmode==bbgd_ftroot )
    {
        ...
        for(j=0; j<=ae_obj_array_get_length(&e->subsolvers)-1; j++)
        {
            ...
            ae_assert((subsolver->subsolverstatus==bbgd_stwaitingforrcomm||subsolver->subsolverstatus==bbgd_stwaitingforsync)||subsolver->subsolverstatus==bbgd_streadytorun, "BBGD: 915110 failed", _state);
        ...
        ae_assert(((e->entrystatus==bbgd_streadytorun&&waitingforrcommcnt+waitingforsynccnt==0)||((e->entrystatus==bbgd_stwaitingforrcomm&&waitingforrcommcnt>0)&&waitingforsynccnt==0))||((e->entrystatus==bbgd_stwaitingforsync&&waitingforrcommcnt==0)&&waitingforsynccnt>0), "BBGD: 919110 failed", _state);
        ...
        do
        {
            ...
            while(j<ae_obj_array_get_length(&e->subsolvers))
            {
                ...
                {
                    e->entrystatus = icase2(e->entrystatus!=bbgd_stunbounded, bbgd_sttimeout, e->entrystatus, _state);
                    front->frontstatus = icase2(front->frontstatus!=bbgd_stunbounded, bbgd_sttimeout, front->frontstatus, _state);
                    result = ae_false;
                    ae_frame_leave(_state);
                    return result;
                }
                if( subsolver->subsolverstatus==bbgd_stwaitingforrcomm )
                {
                    ae_assert(e->entrystatus!=bbgd_stwaitingforsync, "BBGD: integrity check 858018 failed", _state);
                ...
                if( subsolver->subsolverstatus==bbgd_stwaitingforsync )
                {
                    ae_assert(e->entrystatus!=bbgd_stwaitingforrcomm, "BBGD: integrity check 858018 failed", _state);
            ...
            if( (e->entrystatus!=bbgd_streadytorun&&e->entrystatus!=bbgd_stwaitingforrcomm)&&e->entrystatus!=bbgd_stwaitingforsync )
            {
             ...
             */
            waitingforrcommcnt = icase2(e->entrystatus==bbgd_stwaitingforrcomm, 1, 0, _state);
        }
    ...
    if( front->frontmode==bbgd_ftdynamic )
    {
        ...
        for(i=0; i<=front->frontsize-1; i++)
        {
            ...
            {
                ae_obj_array_get(&e->subsolvers, j, &_subsolver, _state);
                ae_assert((subsolver->subsolverstatus==bbgd_stwaitingforrcomm||subsolver->subsolverstatus==bbgd_stwaitingforsync)||subsolver->subsolverstatus==bbgd_streadytorun, "BBGD: 713006 failed", _state);
                if( subsolver->subsolverstatus==bbgd_stwaitingforrcomm )
                {
                    waitingforrcommcnt = waitingforrcommcnt+1;
                }
            ...
            ae_assert(((e->entrystatus==bbgd_streadytorun&&waitingforrcommcnt+waitingforsynccnt==0)||((e->entrystatus==bbgd_stwaitingforrcomm&&waitingforrcommcnt>0)&&waitingforsynccnt==0))||((e->entrystatus==bbgd_stwaitingforsync&&waitingforrcommcnt==0)&&waitingforsynccnt>0), "BBGD: 665242 failed", _state);
        }
        ...
        do
        {
            ...
            while(ae_obj_array_get_length(&state->bbsubproblems)>0&&front->frontsize<state->bbgdgroupsize)
            {
                ...
                if( (state->hasprimalsolution&&!p->donotfathom)&&ae_fp_greater_eq(p->dualbound,state->fprim-state->pdgap*rmaxabs2(state->fprim, (double)(1), _state)) )
                {
                    if( state->dotrace )
                    {
            ...
            for(i=0; i<=front->frontsize-1; i++)
            {
                ...
                while(j<ae_obj_array_get_length(&e->subsolvers))
                {
                    ...
                    if( subsolver->subsolverstatus==bbgd_sttimeout )
                    {
                        ...
                        e->entrystatus = icase2(e->entrystatus!=bbgd_stunbounded, bbgd_sttimeout, bbgd_stunbounded, _state);
                        front->frontstatus = icase2(front->frontstatus!=bbgd_stunbounded, bbgd_sttimeout, bbgd_stunbounded, _state);
                        result = ae_false;
                        ae_frame_leave(_state);
                        return result;
                    }
                    if( subsolver->subsolverstatus==bbgd_stwaitingforrcomm )
                    {
                        ae_assert(e->entrystatus!=bbgd_stwaitingforsync, "BBGD: integrity check 071024 failed", _state);
                        e->entrystatus = bbgd_stwaitingforrcomm;
                        j = j+1;
                        continue;
                    }
                    if( subsolver->subsolverstatus==bbgd_stwaitingforsync )
                    {
                        ae_assert(e->entrystatus!=bbgd_stwaitingforrcomm, "BBGD: integrity check 078024 failed", _state);
                        e->entrystatus = bbgd_stwaitingforsync;
                        j = j+1;
                        continue;
                    }
             ...
             * Trace
             */
            if( state->dolaconictrace&&ae_ifloor(stimergetmsrunning(&state->timerglobal, _state)/(double)bbgd_laconicreportperiod, _state)>state->lastlaconicreportepoch )
            {
                bbgd_tracelaconic(state, _state);
                state->lastlaconicreportepoch = ae_ifloor(stimergetmsrunning(&state->timerglobal, _state)/(double)bbgd_laconicreportperiod, _state);
            }
     ...
     ae_state *_state)
{
    ...
    if( front->frontmode==bbgd_ftroot||front->frontmode==bbgd_ftdynamic )
    {
        ...
        ae_assert((s->subsolverstatus==bbgd_streadytorun||s->subsolverstatus==bbgd_stwaitingforrcomm)||s->subsolverstatus==bbgd_stwaitingforsync, "BBGD: 979201 failed", _state);
     ...
     ae_state *_state)
{
    ...
    {
        ae_obj_array_get(&front->entries, i, &_e, _state);
        ae_assert((((e->entrystatus==bbgd_streadytorun||e->entrystatus==bbgd_stwaitingforrcomm)||e->entrystatus==bbgd_stsolved)||e->entrystatus==bbgd_sttimeout)||e->entrystatus==bbgd_stunbounded, "BBGD: integrity check 304325 failed", _state);
        if( e->entrystatus==bbgd_stwaitingforrcomm )
        {
            if( front->frontmode==bbgd_ftroot||front->frontmode==bbgd_ftdynamic )
            {
     ...
     ae_state *_state)
{
    ...
    bbgd_entrypreparex(entry, front, state, ae_true, rootsubproblem->leafid, _state);
     ...
     ae_state *_state)
{
    ...
    if( !done&&!s->bestdualisintfeas )
    {
        ...
        for(choiceidx=0; choiceidx<=cntchosen-1; choiceidx++)
        {
            ...
            bcopyv(n, &s->subproblemlinearity, &entry->tmpsubproblem.parentlinearity, _state);
     ...
     ae_state *_state)
{
    ...
    hqrndseed(1+subproblemid, 1+17*subproblemid, &entry->entryrng, _state);
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=state->n-1; i++)
    {
        if( state->islinear.ptr.p_bool[i]||subsolver->subproblem.parentlinearity.ptr.p_bool[i] )
        {
    ...
    if( subsolver->subproblem.branchbucket>=0 )
    {
        ...
        {
            i = ae_maxint(i, ae_round(state->nonrootmaxitsaboveaverage*(double)state->globalsynchronizednfev/(double)state->globalsynchronizednsubproblems, _state), _state);
        }
        minnlcsetcond3(&subsolver->nlpsubsolver, state->epsf, state->epsx, i, _state);
        minnlcsetfsqpadditsforctol(&subsolver->nlpsubsolver, state->nonrootadditsforfeasibility, state->ctol, _state);
    }
    minnlcsetearlystopping(&subsolver->nlpsubsolver, &state->isintegral, subsolver->subproblem.parentfdual, state->fprim, 100+5*state->n, 1.0E-4, _state);
    if( state->subsolveralgo==0 )
    {
        minnlcsetalgonlpipm(&subsolver->nlpsubsolver, state->subsolvermemlen, _state);
    }
    if( state->subsolveralgo==1 )
    {
...
lbl_4:
     ...
     */
    if( !_rcommstate_apply_handler_to(&state->rcommv2, &subsolver->nlpsubsolver.rcommv2, _state) )
    {
    ...
    if( subsolver->rstate.rcomm2_handler!=NULL && subsolver->rstate.requesttype!=0 && subsolver->rstate.requesttype<=_ALGLIB_MAX_RCOMMV2_REQUEST )
        subsolver->rstate.rcomm2_handler(&subsolver->rstate, subsolver->rstate.handler_p0, subsolver->rstate.handler_p1, subsolver->rstate.handler_p2, subsolver->rstate.handler_p3, _state);
    else
        goto lbl_rcomm;
lbl_0:
lbl_10:
...
lbl_5:
    ...
    else
    {
         ...
         */
        bbgd_analyzenlpsolutionandenforceintegrality(entry, &subsolver->xsol, &subsolver->nlpsubsolver.repnonlinearityreport, !forbidrecognizingintegrality, &subsolver->nlprep, subsolver->nlpsubsolver.repearlyerrorestimate, state, &entry->solutions, ae_maxint(subsolver->subproblem.branchbucket, 0, _state), uselock, _state);
        subsolver->subsolverstatus = bbgd_stsolved;
        done = ae_true;
    }
lbl_2:
        ...
        for(i=0; i<=solcnt-1; i++)
        {
            ...
            if( state->isintegral.ptr.p_bool[sol->branchvar] )
            {
                ...
                if( sol->branchbucket%2==0 )
                {
                    ...
                    {
                        vdown = ae_maxreal(sol->bestfdual-sol->parentfdual-sol->bestfdualearlyerror-state->epsf*rmaxabs3(sol->bestfdual, sol->parentfdual, (double)(1), _state), (double)(0), _state)/ae_maxreal(vfrac, ae_machineepsilon, _state);
                    }
                ...
                if( sol->branchbucket%2==1 )
                {
                    ...
                    if( sol->hasdualsolution )
                    {
                        vup = ae_maxreal(sol->bestfdual-sol->parentfdual-sol->bestfdualearlyerror-state->epsf*rmaxabs3(sol->bestfdual, sol->parentfdual, (double)(1), _state), (double)(0), _state)/ae_maxreal(vfrac, ae_machineepsilon, _state);
                    }
     ...
     ae_state *_state)
{
    ...
    bbgd_analyzeqpsolutionandenforceintegrality(entry, &subsolver->tmp0, &subsolver->tmp1, terminationtype, state, subproblemtoupdate, uselock, &isintfeasible, _state);
    ...
    if( !isintfeasible&&ae_fp_less_eq(mxsol,state->ctol) )
    {
        ...
        if( tmpterminationtype>=0 )
        {
            ...
            if( terminationtype>0 )
            {
                ...
                bbgd_analyzeqpsolutionandenforceintegrality(entry, &subsolver->tmp0, &subsolver->tmp3, terminationtype, state, subproblemtoupdate, uselock, &isintfeasible, _state);
            }
        }
    }
    ae_frame_leave(_state);
    ...
    *isintfeas = ae_false;
    ...
    if( terminationtype>0&&ae_fp_less_eq(maxerr,state->ctol) )
    {
        ...
        *isintfeas = ae_true;
        ...
        bbgd_subproblemmergeinsolution(subproblem, xsol, nlrep, f, 0.0, maxerr, *isintfeas, ae_false, 0, 0, _state);
        if( uselock )
        {
     ...
     ae_state *_state)
{
    ...
    if( rep->terminationtype==22 )
    {
        ...
        bbgd_subproblemmergeinsolution(subproblem, xsol, nlrep, rep->f, earlyerror, 0.0, ae_false, ae_true, rep->terminationtype, rep->iterationscount, _state);
        if( uselock )
        {
            ae_weak_atomic_release_lock(&entry->entrylock, 1, 0);
        }
    ...
    if( rep->terminationtype>0&&ae_fp_less_eq(rep->sclfeaserr,state->ctol) )
    {
        ...
        bbgd_subproblemmergeinsolution(subproblem, xsol, nlrep, rep->f, earlyerror, rep->sclfeaserr, isintfeas&&!forbidrecognizingintegrality, ae_false, rep->terminationtype, rep->iterationscount, _state);
        if( uselock )
        {
            ae_weak_atomic_release_lock(&entry->entrylock, 1, 0);
        }
...
static void bbgd_tracelaconicheader(bbgdstate* state, ae_state *_state)
{
    ...
    ae_trace("                                                       Nodes statistics  \n");
    ae_trace("       Time         Dual bound       Primal bound      solved     in heap\n");
}
...
static void bbgd_tracelaconic(bbgdstate* state, ae_state *_state)
{
    ...
    ae_trace("    %8d",
        (int)(state->repnsubproblems));
    ae_trace("    %8d",
        (int)(ae_obj_array_get_length(&state->bbsubproblems)));
    ae_trace("\n");
}
...
void _bbgdsubproblem_init_copy(void* _dst, const void* _src, ae_state *_state, ae_bool make_automatic)
{
    ...
    dst->branchbucket = src->branchbucket;
    dst->parentfdual = src->parentfdual;
    ae_vector_init_copy(&dst->parentlinearity, &src->parentlinearity, _state, make_automatic);
    dst->branchvar = src->branchvar;
    dst->branchval = src->branchval;
    dst->ncuttingplanes = src->ncuttingplanes;
    dst->hasprimalsolution = src->hasprimalsolution;
    ...
    dst->earlystopped = src->earlystopped;
    dst->donotfathom = src->donotfathom;
    ae_vector_init_copy(&dst->subproblemlinearity, &src->subproblemlinearity, _state, make_automatic);
}
...
void _bbgdfrontentry_init_copy(void* _dst, const void* _src, ae_state *_state, ae_bool make_automatic)
{
    ...
    _rcommstate_init_copy(&dst->rstate, &src->rstate, _state, make_automatic);
    _hqrndstate_init_copy(&dst->entryrng, &src->entryrng, _state, make_automatic);
     ...
     ae_state *_state)
{
    ...
    nmultistarts = 1;
    timeout = 0;
    bbgdcreatebuf(n, &buf->wrkbndl, &buf->wrkbndu, &state->s, x0, &state->isintegral, &state->isbinary, &buf->tmpb, &state->rawa, &state->rawal, &state->rawau, &state->lcsrcidx, state->lccnt, &state->nl, &state->nu, 0, bbgdsparserequests, bbgdgroupsize, nmultistarts, timeout, 0, &buf->bbgdsubsolver, _state);
     ...
     ae_state *_state)
{
    ...
    ae_assert(memlen>=0, "MINLPSolverSetBBSYNCSubsolverIPM: MemLen<0", _state);
     ...
     ae_state *_state)
{
    ...
    ae_assert(k>=0&&k<state->n, "MINLPSolverMarkAsLinearVar: K is outside of [0,N)", _state);
...
ae_bool minlpsolveriteration(minlpsolverstate* state, ae_state *_state)
{
    ...
    if( state->tracelevel==1 )
    {
        ...
        ae_trace("* %0d variables, including %0d integral\n",
            (int)(n),
            (int)(k));
        k = 0;
        for(i=0; i<=n-1; i++)
    ...
    if( state->algoidx==0 )
    {
        ...
        bbgdcreatebuf(n, &state->bndl, &state->bndu, &state->s, &state->x0, &state->isintegral, &state->isbinary, &state->islinear, &state->xlc.effsparsea, &state->xlc.effal, &state->xlc.effau, &state->xlc.lcsrcidx, state->xlc.nsparse+state->xlc.ndense, &state->nl, &state->nu, state->nnlc, state->issuesparserequests, state->bbgdgroupsize, state->nmultistarts, state->timeout, state->tracelevel, state->bbgdsubsolver, _state);
        ...
        _rcommstate_forward_handler(&state->rcommv2, &state->bbgdsubsolver->rcommv2);
        done = ae_true;
    }
    if( state->algoidx==1 )
    {
...
lbl_4:
    ...
    ae_assert(!_rcommstate_has_handler(&state->rcommv2)||!bbgdisrequestfromfront(state->bbgdsubsolver, _state), "MINLPSOLVERS: integrity check 964031 failed (front-issued RCommV2 request escaped handler passed to BBSYNC)", _state);
    
    /*
     * Offload request
     */
    bbgdoffloadrcommrequest(state->bbgdsubsolver, &originalrequesttype, &state->rcommv2.querysize, &state->rcommv2.queryfuncs, &state->rcommv2.queryvars, &state->rcommv2.querydim, &state->rcommv2.queryformulasize, &state->rcommv2.querydata, _state);
    ae_assert((originalrequesttype==1&&state->issuesparserequests)||(originalrequesttype==2&&!state->issuesparserequests), "MINLPSOLVERS: integrity check 328345 failed", _state);
    ...
    if( densejac )
    {
        ...
        rallocv(state->rcommv2.queryfuncs*state->rcommv2.queryvars*state->rcommv2.querysize, &state->rcommv2.replydj, _state);
    }
    rallocv(state->rcommv2.queryfuncs*state->rcommv2.querysize, &state->rcommv2.replyfi, _state);
    ...
    if( state->rcommv2.rcomm2_handler!=NULL && state->rcommv2.requesttype!=0 && state->rcommv2.requesttype<=_ALGLIB_MAX_RCOMMV2_REQUEST )
        ...
        goto lbl_rcomm;
lbl_0:
    bbgdloadrcommreply(state->bbgdsubsolver, originalrequesttype, state->rcommv2.querysize, state->rcommv2.queryfuncs, state->rcommv2.queryvars, state->rcommv2.querydim, state->rcommv2.queryformulasize, &state->rcommv2.replyfi, &state->rcommv2.replydj, &state->rcommv2.replysj, _state);
    goto lbl_4;
lbl_5:
    done = ae_true;
lbl_2:
...
lbl_8:
    ...
    rcopyallocv(state->rcommv2.querysize*(state->rcommv2.queryvars+state->rcommv2.querydim), &state->mirbfvnssubsolver->querydata, &state->rcommv2.querydata, _state);
    ...
    if( state->rcommv2.rcomm2_handler!=NULL && state->rcommv2.requesttype!=0 && state->rcommv2.requesttype<=_ALGLIB_MAX_RCOMMV2_REQUEST )
        ...
        goto lbl_rcomm;
lbl_1:
    rcopyv(state->rcommv2.querysize*state->rcommv2.queryfuncs, &state->rcommv2.replyfi, &state->mirbfvnssubsolver->replyfi, _state);
    goto lbl_8;
lbl_9:
    done = ae_true;
lbl_6:
