...
// ============================================================================
// Priority Area Statistics / 优先区域面积统计
// ============================================================================

int ExtractPriorityAreas::priorityAreaStats(const std::string& qstrRankingMapPath)
{
	LOG_INFO("priorityAreaStats: Starting...");

	if (qstrRankingMapPath.empty())
	{
		LOG_ERROR("priorityAreaStats: Ranking map path is empty");
		return 1;
	}
	...
	GDALDataset *poDS = (GDALDataset *)GDALOpen(qstrRankingMapPath.c_str(), GA_ReadOnly);
	if (!poDS)
	{
		LOG_ERROR("priorityAreaStats: Cannot open ranking map: " + qstrRankingMapPath);
		return 1;
	}
	...
	if (!poBand)
	{
		LOG_ERROR("priorityAreaStats: Cannot get raster band");
		GDALClose(poDS);
		return 1;
	}
	...
	if (nCols <= 0 || nRows <= 0)
	{
		LOG_ERROR("priorityAreaStats: Invalid raster dimensions");
		GDALClose(poDS);
		return 1;
	}

	// Get NoData value / 获取 NoData 值
	int bNoDataValid = 0;
	double dNoData = poBand->GetNoDataValue(&bNoDataValid);

	// Get GeoTransform for pixel area calculation / 获取 GeoTransform 用于像元面积计算
	double gt[6];
	if (poDS->GetGeoTransform(gt) == CE_Failure)
	{
		LOG_ERROR("priorityAreaStats: Cannot get GeoTransform from ranking map");
		GDALClose(poDS);
		return 1;
	}

	// Calculate single pixel area (km²) / 计算单像元面积 (km²)
	double dPixelWidth = fabs(gt[1]);   // X resolution
	double dPixelHeight = fabs(gt[5]);  // Y resolution (negative in most cases, take abs)
	double dPixelAreaKm2 = 0.0;

	// Check if CRS is geographic (lat/lon) → need cos(latitude) correction
	// 检查是否为经纬度投影 → 需要 cos(纬度) 修正
	const char *pszProj = poDS->GetProjectionRef();
	bool bIsGeographic = false;
	if (pszProj && pszProj[0] != '\0')
	{
		OGRSpatialReference oSRS;
		char *pszWkt = const_cast<char*>(pszProj);
		oSRS.importFromWkt(&pszWkt);
		bIsGeographic = oSRS.IsGeographic() != 0;
	}

	if (bIsGeographic)
	{
		// Geographic CRS: degrees → km conversion with cos(center latitude) correction
		// 经纬度投影：度 → km 换算，使用 cos(中心纬度) 修正
		double dCenterLat = gt[3] + (nRows / 2.0) * gt[5];  // Center latitude
		double dCosLat = cos(dCenterLat * M_PI / 180.0);
		// 1 degree ≈ 111.32 km at equator
		dPixelAreaKm2 = dPixelWidth * 111.32 * dPixelHeight * 111.32 * dCosLat;
		LOG_INFO("priorityAreaStats: Geographic CRS detected, center_lat=" + to_string(dCenterLat) +
			", cos_lat=" + to_string(dCosLat));
	}
	else
	{
		// Projected CRS: units are meters → convert to km²
		// 投影坐标系：单位为米 → 换算为 km²
		dPixelAreaKm2 = dPixelWidth * dPixelHeight / 1e6;
		LOG_INFO("priorityAreaStats: Projected CRS detected, pixel_area_km2=" + to_string(dPixelAreaKm2));
	}

	// Read raster data / 读取栅格数据
	unsigned char *pData = new unsigned char[nCols * nRows];
	if (poBand->RasterIO(GF_Read, 0, 0, nCols, nRows, pData, nCols, nRows, GDT_Byte, 0, 0) != CE_None)
	{
		LOG_ERROR("priorityAreaStats: Failed to read raster data");
		delete[] pData;
		GDALClose(poDS);
		return 1;
	}

	// Count pixels per level (0-6) / 统计各等级像元数
	long long levelCounts[7] = {0, 0, 0, 0, 0, 0, 0};
	long long nTotalValid = 0;

	for (int i = 0; i < nCols * nRows; i++)
	{
		unsigned char val = pData[i];
		// Skip NoData pixels / 跳过 NoData 值
		if (bNoDataValid && (double)val == dNoData) continue;
		// Count valid pixels / 统计有效像元
		if (val >= 0 && val <= 6)
		{
			levelCounts[val]++;
			nTotalValid++;
	...
	// Calculate areas and percentages / 计算面积和占比
	double dTotalAreaKm2 = nTotalValid * dPixelAreaKm2;
	long long nPriorityCount = 0;
	for (int lvl = 1; lvl <= 6; lvl++) nPriorityCount += levelCounts[lvl];
	double dPriorityAreaKm2 = nPriorityCount * dPixelAreaKm2;
	double dPriorityPct = (nTotalValid > 0) ? (nPriorityCount * 100.0 / nTotalValid) : 0.0;

	// Output results to stdout (key=value format) / 输出结果到 stdout (key=value 格式)
	cout << fixed << setprecision(4);
	for (int lvl = 0; lvl <= 6; lvl++)
	{
		double dAreaKm2 = levelCounts[lvl] * dPixelAreaKm2;
		cout << "level_" << lvl << "_count=" << levelCounts[lvl]
			<< " level_" << lvl << "_area=" << dAreaKm2 << " ";
	}
	cout << "total_count=" << nTotalValid
		<< " total_area=" << dTotalAreaKm2
		<< " priority_count=" << nPriorityCount
		<< " priority_area=" << dPriorityAreaKm2
		<< " priority_pct=" << dPriorityPct << endl;

	// Log result / 记录结果
	LOG_RESULT("priority-stats", "priority_pct", dPriorityPct);
	LOG_RESULT("priority-stats", "priority_area_km2", dPriorityAreaKm2);
	LOG_RESULT("priority-stats", "total_area_km2", dTotalAreaKm2);

	LOG_INFO("priorityAreaStats: Completed.");
	return 0;
}
